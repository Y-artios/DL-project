...
def main() -> None:
     #Initialize the super-resolution bsrresnet_model
    ...
    # Load the super-resolution bsrresnet_model weights
    checkpoint = torch.load(srresnet_config.model_weights_path, map_location=lambda storage, loc: storage)
    g_model.load_state_dict(checkpoint["state_dict"])
    print(f"Load `{srresnet_config.g_arch_name}` model weights "
          f"`{os.path.abspath(srresnet_config.model_weights_path)}` successfully.")

    # Create a folder of super-resolution experiment results
    make_directory(srresnet_config.sr_dir)

    # Start the verification mode of the bsrresnet_model.
    g_model.eval()
